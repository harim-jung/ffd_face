#!/usr/bin/env python3
# coding: utf-8
# for full 3DMM parameters (7 + 199 + 29)
...
class DeformVDCLoss(nn.Module):
    def __init__(self):
        ...
        self.param_mean = _to_tensor(param_mean).double()
        self.param_std = _to_tensor(param_std).double()

        self.u = _to_tensor(mean).double()
        self.w_shp = _to_tensor(w_shp_full).double()
        self.w_exp = _to_tensor(w_exp_full).double()

        self.deform_matrix = _to_tensor(deform_matrix_f) # (38365, 216)
        self.control_points = _to_tensor(control_points_f) # (216, 3)
    ...
    def deform_mesh(self, param, batch):
        deform = param.view(batch, cp_num_f//3, -1) # reshape to 3d
