...
def parse_args():
    parser = argparse.ArgumentParser(description='3DMM Fitting (VDC)')
    parser.add_argument('-j', '--workers', default=8, type=int)
    parser.add_argument('--epochs', default=50, type=int)
    parser.add_argument('--start-epoch', default=1, type=int)
    parser.add_argument('-b', '--batch-size', default=256, type=int)
    parser.add_argument('-vb', '--val-batch-size', default=32, type=int)
    parser.add_argument('--base-lr', '--learning-rate', default=0.000001, type=float)
    parser.add_argument('--momentum', default=0.9, type=float, metavar='M')
    parser.add_argument('--weight-decay', '--wd', default=0.0005, type=float)
    parser.add_argument('--print-freq', '-p', default=50, type=int)
    parser.add_argument('--resume', default='snapshot/phase1_wpdc_lm_checkpoint_epoch_20.pth.tar', type=str, metavar='PATH')
    ...
    parser.add_argument('--snapshot', default='snapshot/phase2_wpdc_lm_vdc_all', type=str)
    parser.add_argument('--log-file', default='training/logs/phase2_wpdc_lm_vdc_all_200202.log', type=str)
    ...
    parser.add_argument('--opt-style', default='all', type=str, help='all or resample')
    parser.add_argument('--resample-num', default=132, type=int)
    parser.add_argument('--loss', default='vdc', type=str, help='wpdc or vdc')
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.loss.lower() == 'vdc':
            ...
            loss = param_loss + torch.abs(param_loss) / torch.abs(lm_loss) * lm_loss
        else:
...
def main():
    ...
    model = getattr(mobilenet_v1_2, args.arch)(param_classes=args.param_classes, lm_classes=args.lm_classes)
...
if __name__ == '__main__':
    writer = SummaryWriter('training/runs/wpdc_lm_vdc_all')
    main()
    writer.close()