...
import models.mobilenet_v1_ffd_feat
import torch.backends.cudnn as cudnn
from torch.utils.tensorboard import SummaryWriter
from collections import OrderedDict

from utils.ddfa import DDFADataset, ToTensorGjz, NormalizeGjz, reconstruct_vertex
from utils.ddfa import str2bool, AverageMeter
from utils.inference import dump_rendered_img
from utils.io import mkdir
from bernstein_ffd.ffd_utils import cp_num, deformed_vert
from losses.cosine_loss import CosineFeatLoss
...
def parse_args():
    ...
    parser.add_argument('-j', '--workers', default=8, type=int) # 0, gpu*4
    ...
    parser.add_argument('--snapshot', default='snapshot/ffd_feat', type=str)
    parser.add_argument('--log-file', default='training/logs/ffd_feat_210301.log', type=str)
...
def train(train_loader, model, criterion, optimizer, epoch, transform):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # render gt mesh on image
        gt_param = target[:, :62]
        # todo - how to deal with batch?
        gt_vert = reconstruct_vertex(gt_param, dense=True, transform=True) # face region only
        gt_img = dump_rendered_img(gt_vert, filename, wfp=None, show_flag=False, alpha=1)

        # render inferenced mesh on image (param_output)
        out_param = output[:, :cp_num]
        df_vert = deformed_vert(out_param, transform=True, face=True)
        df_img = dump_rendered_img(df_vert, filename, wfp=None, show_flag=False, alpha=1)


        with torch.no_grad():
            # extract feature from rendered gt image 
            gt_img = transform(gt_img).unsqueeze(0)
            target = model(gt_img)
            target_feat = target[:, cp_num:] 

            # extract feature from rendered inferenced image 
            df_img = transform(df_img).unsqueeze(0)
            output = model(df_img)
            output_feat = output[:, cp_num:] 


        data_time.update(time.time() - end)

        loss = criterion(output_feat, target_feat)
...
def validate(val_loader, model, criterion, epoch, transform):
    model.eval()
...
def main():
    ...
    model = getattr(mobilenet_v1_ffd_feat, args.arch)(param_classes=args.param_classes)
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, transform)
        ...
        validate(val_loader, model, criterion, epoch, transform)


if __name__ == '__main__':
    writer = SummaryWriter('training/runs/ffd_feat')
    main()
    writer.close()