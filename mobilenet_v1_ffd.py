...
class MobileNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # return torch.cat((param, lm), dim=1)
        return param
...
def mobilenet_2(param_classes=192, input_channel=3):
    model = MobileNet(widen_factor=2.0, param_classes=param_classes, input_channel=input_channel)
    return model


def mobilenet_1(param_classes=192, input_channel=3):
    model = MobileNet(widen_factor=1.0, param_classes=param_classes, input_channel=input_channel)
...
if __name__ == '__main__':
    model = mobilenet_1(param_classes=24000)
    transform = transforms.Compose([ToTensorGjz(), NormalizeGjz(mean=127.5, std=128)])
    x = cv2.imread('samples/outputs/300w-lp/LFPWFlip_LFPW_image_train_0812_1_1_gt.jpg')
    x = transform(x).unsqueeze(0)
    y = model(x)

    x = cv2.imread('samples/outputs/300w-lp/LFPWFlip_LFPW_image_train_0812_1_1.jpg')
    x = transform(x).unsqueeze(0)
    y_ = model(x)

    x = cv2.imread('../Datasets/train_aug_120x120/LFPWFlip_LFPW_image_train_0812_1_1.jpg')
    x = transform(x).unsqueeze(0)
    y_2 = model(x)
