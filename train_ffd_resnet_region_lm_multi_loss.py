...
def parse_args():
    ...
    parser.add_argument('--snapshot', default='snapshot/ffd_resnet_region_lm_learn', type=str)
    parser.add_argument('--log-file', default='training/logs/ffd_resnet_region_lm_learn_210401.log', type=str)
    ...
    parser.add_argument('--loss', default='vdc_region_lm_l1', type=str)
    parser.add_argument('--weights', default='learnable', type=str)
...
def train(train_loader, model, criterion, vertex_criterion, lm_criterion, optimizer, epoch, log_vars):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        output = model(input)
        log_vars_exp = torch.exp(-log_vars)
        ...
        loss_multi = torch.sum(log_vars_exp * loss_group + log_vars)
        # loss_multi = torch.sum(log_vars_exp * loss_group + log_vars, -1)
        ...
        losses.update(loss_multi.item(), input.size(0))
        # losses.update(loss.item(), input.size(0))
        param_losses.update(vertex_loss.item(), input.size(0))
        lm_losses.update(loss_group[1:].mean().item(), input.size(0))
        ...
        if i > 0 and i % args.print_freq == 0:
            logging.info(f'Epoch: [{epoch}][{i}/{len(train_loader)}]\t'
                         ...
                         f'Vertex Loss {param_losses.val:.4f} ({param_losses.avg:.4f})\t'
                         f'Loss {losses.val:.4f} ({losses.avg:.4f})\t'
                         f'Weight {log_vars_exp}\t'
...
def validate(val_loader, model, criterion, vertex_criterion, lm_criterion, epoch, log_vars, log=True):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            losses.append(loss_multi.item())
            lm_losses.append(loss_group[1:].mean().item())
            param_losses.append(vertex_loss.item())
...
def main():
    ...
    log_vars = nn.Parameter(torch.zeros(10, requires_grad=True, device="cuda"))
    criterion = DeformVDCLoss().cuda()
    vertex_criterion = VertexOutput().cuda()
    lm_criterion = RegionLMLoss().cuda()

    # optimizer = torch.optim.Adam([model.parameters(), log_vars], lr=args.base_lr)
    optimizer = torch.optim.Adam([{'params':model.parameters(), 'lr':args.base_lr}, {'params':log_vars, 'lr':args.base_lr}])
    ...
    if args.test_initial:
        ...
        validate(val_loader, model, criterion, vertex_criterion, lm_criterion, args.start_epoch, log_vars, log=False)
    ...
    # for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        # train for one epoch
        train(train_loader, model, criterion, vertex_criterion, lm_criterion, optimizer, epoch, log_vars)
        ...
        validate(val_loader, model, criterion, vertex_criterion, lm_criterion, epoch, log_vars)


if __name__ == '__main__':
    writer = SummaryWriter('training/runs/ffd_resnet_region_lm_learn')
    main()
    writer.close()