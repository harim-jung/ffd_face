...
def parse_args():
    ...
    parser.add_argument('--snapshot', default='snapshot/ffd_resnet_region_lm_0.46_adamw', type=str)
    parser.add_argument('--log-file', default='training/logs/ffd_resnet_region_lm_0.46_adamw_210330.log', type=str)
...
def train(train_loader, model, criterion, vertex_criterion, lm_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        lm_group = torch.tensor([up_mouth, low_mouth, up_nose, low_nose, l_brow, r_brow, l_eye, r_eye, contour])
        ...
        lm_losses.update(lm_group.mean(), input.size(0))
...
def validate(val_loader, model, criterion, vertex_criterion, lm_criterion, epoch, log=True):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            lm_losses.append(lm_group.mean())
...
if __name__ == '__main__':
    writer = SummaryWriter('training/runs/ffd_resnet_region_lm_0.46_adamw')
    main()
    writer.close()