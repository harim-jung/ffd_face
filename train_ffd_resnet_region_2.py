...
def parse_args():
    ...
    # parser.add_argument('--resume', default='snapshot/ffd_resnet_region/ffd_resnet_region_checkpoint_epoch_3.pth.tar', type=str, metavar='PATH')
    ...
    parser.add_argument('--snapshot', default='snapshot/ffd_resnet_region_ratio_2', type=str)
    parser.add_argument('--log-file', default='training/logs/ffd_resnet_region_ratio_2_210324.log', type=str)
...
def train(train_loader, model, criterion, vertex_criterion, mouth_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # loss = 0.8 * vertex_loss + 0.2 * lm_loss
        loss = mouth_loss

        losses.update(loss.item(), input.size(0))
        # rest_losses.update(rest_loss.item(), input.size(0))
...
def validate(val_loader, model, criterion, vertex_criterion, mouth_criterion, epoch, log=True):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            # rest_losses.append(rest_loss.item())
...
def main():
    ...
    if args.test_initial:
        ...
        validate(val_loader, model, criterion, vertex_criterion, mouth_criterion, args.start_epoch, log=False)
    ...
    # for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        # train for one epoch
        train(train_loader, model, criterion, vertex_criterion, mouth_criterion, optimizer, epoch)
        ...
        validate(val_loader, model, criterion, vertex_criterion, mouth_criterion, epoch)


if __name__ == '__main__':
    writer = SummaryWriter('training/runs/ffd_resnet_region_ratio_2')
    main()
    writer.close()