...
def parse_args():
    ...
    parser.add_argument('--snapshot', default='snapshot/ffd_efficientnet_no_pose_axis_angle_abss', type=str)
    parser.add_argument('--log-file', default='training/logs/ffd_efficientnet_no_pose_axis_angle_abss_210426.log', type=str)
...
def main():
    ...
    if args.arch.startswith("mobilenet"):
        if args.arch.endswith("v1"):
            model = getattr(mobilenet_v1_ffd, args.arch)(param_classes=args.param_classes)
        elif args.arch.endswith("v3"):
            model = torchvision.models.mobilenet_v3_large(pretrained=False, num_classes=args.param_classes)
...
if __name__ == '__main__':
    writer = SummaryWriter('training/runs/ffd_efficientnet_no_pose_axis_angle_abss')
    main()
    writer.close()